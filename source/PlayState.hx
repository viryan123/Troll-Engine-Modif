...
class PlayState extends MusicBeatState
{
	...
	public static var downScrollStrum:Bool = false;
	...
	override public function create()
	{
		...
		downScrollStrum = ClientPrefs.downScroll;
		...
		if(ret != Globals.Function_Stop && !transitioning) {
			...
			if (SONG.validScore){
				...
				if (!playOpponent && saveScore && ratingFC!='Fail')
					Highscore.saveScore(SONG.song, songScore, percent, difficulty);
			}
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (!playOpponent){
						if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
							Highscore.saveWeekScore(WeekData.getWeekFileName(), campaignScore, difficulty);
...
/* 		if(inst.fadeTween != null) {
	...
	public function RecalculateRating() {
		...
		if(ret != Globals.Function_Stop)
		{
			...
			if (judges.get("good") > 0) ratingFC = "GFC";
	...
	public function newShader(fragFile:String = null, vertFile:String = null)
	{
